...
io_vec::io_vec(bool _single, size_t io_num, bool _endian){

	if(Layout::numNodes() % io_num != 0){                                                                                                                                         
...
void io_vec::writeR(FILE* filehand){
    QMP_barrier();
    if(single) writeRF(filehand);
    else writeRD(filehand);
    QMP_barrier();
}
...
void io_vec::writeRD(FILE* filehand){
	...
	for(size_t site=0;site<para.vvol;site+=para.xinz){
		...
		for(size_t j=0;j<para.xinz;j++){
			size_t linear = adlinearSiteIndex(adcrtesn(IO_idx*para.vvol+site+j));
			memcpy(rin+j*para.memsize, (char *)&(realD.elem(0))+linear*para.memsize, para.memsize);
		}
		para.readtime6.stop();
		para.readtime5.start();
		if(node != IO_idx*para.io_ratio) QDPInternal::route((void *)rin, node, IO_idx*para.io_ratio, para.xinz*para.memsize);
		if(para.io_idx == IO_idx) memcpy(rout+site,rin,para.memsize*para.xinz);
		para.readtime5.stop();
	}
	free(rin);
	rin = NULL;
	if(para.io_flag){
		...
		para.readtime1.start();
		adfseek(filehand, para.io_idx*para.vvol*para.memsize, SEEK_CUR);
		fwrite((char*)rout, 1, para.vvol*para.memsize, filehand);
		adfseek(filehand, (para.io_num-para.io_idx-1)*para.vvol*para.memsize, SEEK_CUR);
		para.readtime1.stop();
		free(rout);
		rout = NULL;
...
void io_vec::writeRF(FILE* filehand){
    ...
    for(size_t site=0;site<para.vvol;site+=para.xinz){
        ...
        for(size_t j=0;j<para.xinz;j++){
            size_t linear = adlinearSiteIndex(adcrtesn(IO_idx*para.vvol+site+j));
            memcpy(rin+j*para.memsize, (char *)&(realF.elem(0))+linear*para.memsize, para.memsize);
        }
        para.readtime6.stop();
        para.readtime5.start();
        if(node != IO_idx*para.io_ratio) QDPInternal::route((void *)rin, node, IO_idx*para.io_ratio, para.xinz*para.memsize);
