...
namespace Chroma
{
  ...
  namespace GridQuarkSourceConstEnv
  {
	...
	link_smearing = LinkSmearingEnv::nullXMLGroup();
      ...
      if (paramtop.count("eo_level") != 0)
           read(paramtop, "eo_level",  eo_level);
      else
           eo_level=0;
	...
	random(rnd1);
        const Real twopi = 6.283185307179586476925286;
	Real twopiN = twopi / 3; //Z_3 grid for bayron;
	...
	multi1d<int> ngrid(4);
	for(int idr=0;idr<4;idr++) ngrid[idr]=(params.t_grid[idr]>0)?(Layout::lattSize()[idr])/params.t_grid[idr]:1;
	...
	c1=c1*phase;
       ...
       {
         if(params.eo_level>0)
         {
          multi1d<int> t0(3);
          for(int idr=0;idr<3;idr++)
             t0[idr]=params.t_srce[idr]%offset[idr];
     	  c=where(
	     ((((Layout::latticeCoordinate(0))-t0[0])/offset[0]+
	       ((Layout::latticeCoordinate(1))-t0[1])/offset[1]+
	       ((Layout::latticeCoordinate(2))-t0[2])/offset[2])%(params.eo_level+1)==0),
       ...
       if(params.eo_level>0)
          QDPIO::cout << "The non-zero eo_level is not applied to the present grid layout" <<   std::endl;
	...
	QDP_abort(1);
      ...
      double value=toDouble(real(sum(corr)));
	...
	QDP_abort(1);
        ...
        LatticeStaggeredPropagator dummy_return_value;
        return dummy_return_value; // remove warnings
    }
  }
}
